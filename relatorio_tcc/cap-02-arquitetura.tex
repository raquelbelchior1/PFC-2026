...
A operação em unidades de instrução militar não pode presumir a
presença de pessoal com formação em desenvolvimento de \textit{software}
capaz de configurar um ambiente Python do zero, tampouco conectividade
constante à internet para \texttt{pip install}. Para eliminar essa
barreira de adoção, o projeto inclui, na pasta \texttt{installer/} do
repositório (Anexo~A), um instalador único e \textbf{100\% offline}
para Windows 10/11 de 64~\textit{bits}, gerado com o
\textit{Inno~Setup} a partir do script \texttt{ARSandbox.iss}. Em uma
máquina limpa, sem qualquer pré-requisito além do próprio Windows, o
instalador executa automaticamente, em sequência:
...
\begin{enumerate}
    ...
    \item Aplica automaticamente, via \texttt{patch\_pykinect2.py}, os
          ...
          do NumPy);
    \item Instala o \textit{Kinect for Windows Runtime v2.0}, necessário
    ...
    \item Executa um teste de fumaça dos módulos importados, registrando
          ...
          \textbf{AR Sandbox} e \textbf{AR Sandbox --- Diagnóstico do
...
o mesmo princípio de \textit{plug \& play} que orienta a cadeia de
...
O sistema apresenta ao operador \textbf{duas janelas OpenCV}
simultâneas, com propósitos distintos e complementares:

\begin{description}
    \item[\texttt{Projecao\_Areia}.] Destinada ao \textbf{projetor}
          ...
          areia.
    \item[\texttt{Gabarito\_MDE}.] Janela de \textbf{controle do
...
aba \emph{Calibração}: modo de calibração (tampa/base), distância do
...
Durante a etapa \emph{opcional} de calibração empírica do projetor
(via \texttt{calibrar\_projetor}, Seção~\ref{subsec:calib-empirica-projetor}), a janela
\texttt{Projecao\_Areia} deixa \textbf{temporariamente} de exibir a
malha de cores e passa a projetar um \textbf{padrão de tabuleiro de
...
malha de cores. Este é, portanto, um mecanismo de \textbf{depuração e
...
parâmetros de projeção obtidos \emph{analiticamente}
...
Gamma et al.~\cite{gamma1995design} identificados na implementação, e o
...
\begin{description}
    \item[Strategy + \textit{Fallback} (\texttt{KinectSensor}).] Cada
          ...
          o \textit{Strategy} com uma \textbf{cadeia de fallback}
    ...
    \item[\textit{State Machine} (\texttt{main.py}).] A execução é
          ...
          \textit{booleanas} soltas (\emph{flags}), que tendem a
