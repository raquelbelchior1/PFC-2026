...
Como verificação de coerência, antes de compor $T_{\text{shift}}$
...
alternativo é exercitado por quatro classes de teste dedicadas
(Capítulo~\ref{cap:resultados}):

\begin{itemize}
    \item \texttt{TestCalibracaoProjetorOpenCV}
    \item \texttt{TestGeracaoTabuleiro}
    \item \texttt{TestCantosParaPontosMesa}
    \item \texttt{TestPipelineCalibracaoProjetor}
\end{itemize}

\noindent mas permanece inativo por padrão: a configuração corrente do
...
\noindent Essa decisão é validada empiricamente: a suíte de 88 testes
