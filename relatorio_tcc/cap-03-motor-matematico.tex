...
sequência: primeiro o \textbf{RANSAC} isola o maior subconjunto de
pontos coplanares (Seção~\ref{subsec:ransac}); em seguida, o
\textbf{SVD} refina a normal com precisão sub-milimétrica, mas apenas
...
detectado à \emph{origem} $(0, 0, 0)$. Entretanto, a convenção interna
de discretização e renderização (Capítulo~\ref{cap:rendering}) adota a
mesa como o domínio $[0, L_x] \times [0, L_y] \times [0, h_{\max}]$, com
a origem $XY$ em um \emph{canto} e a cota $Z = 0$ fixada na \textbf{base
...
$x < 0$ ou $y < 0$) seria projetada fora do domínio e \emph{colapsada
...
ajuste em $Z$, a cota zero coincidiria com o \emph{plano de calibração}
em vez da base do caixão — o que é apenas correto quando esse plano é a
própria base (modo \textbf{base vazia}), mas não quando ele é a tampa
apoiada $h_{\max}$ acima dela (modo \textbf{tampa}, oficial).
...
modo base, na cota $0$ (o fundo, que \emph{é} o próprio plano de
...
aplica um filtro \textit{\textbf{dentro}} que descarta pontos cujas
...
Como \textbf{validação de sanidade}, antes de compor $T_{\text{shift}}$
...
disponibiliza um caminho alternativo de \textbf{estimação empírica} dos
parâmetros de Tsai, no qual a etapa de otimização que o artigo original
de Tsai~\cite{tsai1987versatile} descreve é de fato executada. A
...
projetor; essa imagem é projetada \textbf{temporariamente} na janela
...
mas permanece \textbf{inativo por padrão}: a configuração corrente do
...
\textbf{compor os dois}, cada um explorando a etapa em que é mais forte:

\begin{itemize}
    \item O \textbf{RANSAC} (Seção~\ref{subsec:ransac}) é robusto a
    ...
    \item O \textbf{SVD}, aplicado apenas sobre os inliers
