...
regra de cor) até a suíte atual, com 88 testes. Para deixar
explícito o que cada teste verifica, a Tabela~\ref{tab:tests}
agrupa as 23 classes em cinco frentes funcionais do \textit{pipeline},
...
\texttt{TestErroQuantitativoElevacao} & Erro do MDE antes da coloração (Seção~\ref{subsec:erro-quantitativo-mde}) & 5 \\
\emph{Subtotal} & & \emph{11} \\
\midrule
\textbf{Total}                        & \multicolumn{2}{r}{\textbf{88}}                \\
\bottomrule
\end{tabularx}
\end{table}
...
reporta 88 testes aprovados, nenhuma falha. O único teste
...
funcional (abril/2026) até os 83 testes de agosto/2026 — marco em que
o gráfico e a Listagem~\ref{lst:saida-testes} abaixo foram capturados
—, sem nenhum retrocesso no total. Cinco casos adicionais
(\texttt{TestErroQuantitativoElevacao}, Seção~\ref{subsec:erro-quantitativo-mde})
foram incorporados posteriormente, em setembro/2026, elevando a suíte
aos 88 testes já refletidos na Tabela~\ref{tab:tests} acima; o gráfico
de evolução não foi reprocessado para incluir esse ponto adicional.
Em qualquer um dos dois marcos, a evidência é a mesma: cada nova
capacidade do
...
por frente funcional detalhada na Tabela~\ref{tab:tests} no marco de
agosto/2026 (83 testes, antes da nova classe de testes incorporada à
quinta frente — Seção~\ref{subsec:erro-quantitativo-mde}), deixando
...
funcionais do \textit{pipeline} no marco de agosto/2026 (Tabela~\ref{tab:tests}
reflete os 88 testes atuais, incluindo os 5 casos incorporados depois
deste gráfico ter sido gerado).}
...
das 88 linhas individuais \texttt{ok} por brevidade editorial (todas
...
\subsubsection{Capacidade da métrica de denunciar falhas reais: controles negativos}
...
Todos os números apresentados nesta seção — 88/88 testes aprovados,
...
e independente de qualquer hardware, pelos 88 testes automatizados
...
executando o próprio \texttt{gerar\_imagem\_grade\_cores} — o mesmo
\textit{pipeline} de renderização de produção — com os parâmetros
provisórios de projetor então usados em modo real, apenas para
simulação ($f_x = f_y = 500$,
...
\begin{enumerate}
    ...
    \item Os parâmetros de projeção do projetor, em modo real, que eram valores provisórios
          utilizados apenas para simulação ($f_x = f_y = 500$,
          $c_x = 320$, $c_y = 240$, $\mathbf{t}_{\text{ext}} = (0,0,1)^{T}$),
