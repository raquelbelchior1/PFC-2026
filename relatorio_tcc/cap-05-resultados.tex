...
regra de cor) até a suíte atual, com \textbf{83 testes}. Para deixar
...
\texttt{TestDiscretizacaoGradeNegativa} & Filtro \textit{dentro} do domínio             & 1 \\
...
\noindent Além da contagem de casos, a \textbf{cobertura de código}
...
código de produção: \textbf{91\%} das linhas de
\texttt{motor\_caixao\_areia.py} e \textbf{48\%} das linhas de
\texttt{mde\_cartografia.py} são executadas durante os testes
(\textbf{75\%} combinado). A lacuna em \texttt{mde\_cartografia.py}
...
reporta \textbf{83} testes aprovados, nenhuma falha. O único teste
...
sintéticas, o \textbf{procedimento de aceitação físico} usado para
...
projetada em \textbf{verde} (a altura medida coincide com a cota zero
alvo); (D) o centro, ainda vazio, deve ser projetado em \textbf{azul}
(falta volume); (E) após a inserção de um cubo físico de 10~cm no
centro, o topo do cubo deve passar a ser projetado em \textbf{verde}, e
...
Contar testes prova apenas que a suíte existe; a \textbf{cobertura de
...
produção é de fato \emph{exercitado} por ela — isto é, quantas linhas
...
(Capítulo~\ref{cap:conclusao}, \emph{Trabalhos Futuros}), permanecem como
...
Em modo simulação, a grade de areia virtual nasce \textbf{vazia}
($Z = 0$ em todas as células, coincidindo com a cota zero na base do
caixão) e é comparada, por padrão, com o mapa sintético
\textbf{Cubo Central} — um bloco de $+0{,}10$~m de altura ocupando o
...
\begin{itemize}
    \item Fora do cubo, malha \textbf{verde} (areia em $Z = 0$,
          coincidindo com o alvo $Z = 0$ dentro da tolerância $\tau$);
    \item No terço central, malha \textbf{azul} (areia em $Z = 0$
...
\textbf{Morro Gaussiano} (pico suave no centro, decaindo às bordas)
...
filtro \textit{dentro} do domínio físico da mesa
