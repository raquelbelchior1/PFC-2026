...
A primeira versão experimental do sistema projetava diretamente
...
discretização em malha regular. Os pontos são agregados em
células retangulares, cada célula é classificada por uma única cor e
desenhada como um polígono preenchido via
\texttt{cv2.fillPoly}.
...
\noindent o que evidencia a filtragem estatística obtida pela
...
\texttt{NaN} e simplesmente não renderizadas no
...
\noindent A interpretação operacional é direta: vermelho
indica excesso de areia (\emph{cavar}), azul indica falta
(\emph{preencher}) e verde indica conformidade com o MDE
...
\emph{frame} é, portanto, proporcional ao número de células da
malha, $O(N_x \cdot N_y)$, sendo realizada uma chamada a
\texttt{fillPoly} para cada célula.

\section{Emulador Interativo de Areia}
\label{sec:emulador}

O emulador interativo existe para resolver um problema prático de
engenharia e de pesquisa: como desenvolver, testar e demonstrar todo o
\textit{pipeline} de Realidade Aumentada — discretização em malha,
comparação com o MDE, projeção de Tsai e rasterização — em qualquer
computador, a qualquer momento, sem depender da disponibilidade
do sensor Kinect físico? Durante a maior parte do ciclo de
desenvolvimento deste projeto, o hardware não estava permanentemente
acessível aos quatro integrantes da equipe, e uma apresentação ou
depuração que dependesse exclusivamente do sensor conectado seria
frágil por construção. O emulador resolve isso substituindo a captura
por hardware por uma matriz de alturas persistente, manipulável em
tempo real pelo \emph{mouse}, que atravessa exatamente o mesmo código
de discretização, comparação e projeção usado no modo real — nenhuma
ramificação condicional separa ``modo simulação'' de ``modo real'' a jusante da captura.
jusante da captura. Esse é, ao mesmo tempo, o nível mais profundo da
cadeia de \textit{fallback} do sistema (ativado quando nenhum
sensor físico é encontrado) e uma ferramenta deliberada de teste
independente: a Seção~\ref{subsec:fallback} detalha os quatro níveis
dessa cadeia e o mecanismo mais amplo de resiliência do qual o
emulador é a camada final.
